object ConnectDB: TConnectDB
  Left = 0
  Top = 0
  Caption = #1055#1086#1076#1082#1083#1102#1095#1077#1085#1080#1103' '#1082' '#1073#1072#1079#1077' '#1076#1072#1085#1085#1099#1093
  ClientHeight = 466
  ClientWidth = 725
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  OnClose = FormClose
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object cxGrid2: TcxGrid
    Left = 8
    Top = 40
    Width = 709
    Height = 253
    TabOrder = 0
    object cxGrid2DBTableView1: TcxGridDBTableView
      DataController.DataSource = DataModule.SourceDB
      DataController.Summary.DefaultGroupSummaryItems = <>
      DataController.Summary.FooterSummaryItems = <>
      DataController.Summary.SummaryGroups = <>
      OptionsData.Editing = False
      OptionsData.Inserting = False
      OptionsSelection.CellSelect = False
      OptionsView.GroupByBox = False
      OptionsView.Indicator = True
      object cxGrid2DBTableView1MONREMOTE_ADDRESS: TcxGridDBColumn
        DataBinding.FieldName = 'MON$REMOTE_ADDRESS'
        Width = 100
      end
      object cxGrid2DBTableView1MONATTACHMENT_NAME: TcxGridDBColumn
        DataBinding.FieldName = 'MON$ATTACHMENT_NAME'
        Width = 150
      end
      object cxGrid2DBTableView1MONUSER: TcxGridDBColumn
        DataBinding.FieldName = 'MON$USER'
        Width = 75
      end
      object cxGrid2DBTableView1MONREMOTE_PROTOCOL: TcxGridDBColumn
        DataBinding.FieldName = 'MON$REMOTE_PROTOCOL'
      end
      object cxGrid2DBTableView1MONTIMESTAMP: TcxGridDBColumn
        DataBinding.FieldName = 'MON$TIMESTAMP'
      end
      object cxGrid2DBTableView1MONREMOTE_PROCESS: TcxGridDBColumn
        DataBinding.FieldName = 'MON$REMOTE_PROCESS'
      end
      object cxGrid2DBTableView1MONREMOTE_PID: TcxGridDBColumn
        DataBinding.FieldName = 'MON$REMOTE_PID'
      end
      object cxGrid2DBTableView1MONCHARACTER_SET_ID: TcxGridDBColumn
        DataBinding.FieldName = 'MON$CHARACTER_SET_ID'
      end
      object cxGrid2DBTableView1MONGARBAGE_COLLECTION: TcxGridDBColumn
        DataBinding.FieldName = 'MON$GARBAGE_COLLECTION'
      end
      object cxGrid2DBTableView1MONSTAT_ID: TcxGridDBColumn
        DataBinding.FieldName = 'MON$STAT_ID'
      end
    end
    object cxGrid2Level1: TcxGridLevel
      GridView = cxGrid2DBTableView1
    end
  end
  object RzRefrash: TRzBitBtn
    Left = 548
    Top = 9
    Width = 95
    Hint = #1054#1073#1085#1086#1074#1080#1090#1100
    Caption = #1054#1073#1085#1086#1074#1080#1090#1100
    ParentShowHint = False
    ShowHint = True
    TabOrder = 1
    OnClick = RzRefrashClick
    Glyph.Data = {
      
      00FF953A9A39C058319543B125B3FF00FFFF00FFFF00FFFF00FF}
  end
end
