...
uses
  ...
  ComCtrls, cxCheckBox, frxExportPDF, frxExportXLS, cxContainer, cxTextEdit;
...
type
  TMain = class(TForm)
    ...
    Action2: TAction;
    Get_Selection: TButton;
    Memo1: TMemo;
    colCheck: TcxGridDBColumn;
    BitGroupPrint: TBitBtn;
    ...
    procedure UmCheck(var Message: TMessage);
...
//        TcxGridTableDataCellViewInfo; var ADone: Boolean);
    ...
    procedure cxGridDBTableView3MouseDown(Sender: TObject; Button: TMouseButton;
...
var
  Main: TMain;
  UserRight, UserIDGlobal,ID_zakaz : integer;
  UserName: string;
...
Memo1.Clear;
  ...
  with cxGrid1DBTableView1.Controller do
    ...
    begin
      ...
      Memo1.Lines.Add(VarToStr(cxGrid1DBTableView1.Controller.SelectedRecords[i].Values[0]));
    end;
    showmessage(s);
    cxGrid1DBTableView1.DataController.DataModeController.SmartRefresh:=False;
    DataModule.DataSetTEST.Close;
    DataModule.DataSetTEST.ParamByName('peremIdArray').AsString := S;
...
procedure TMain.Check(AGridView: TcxGridDBTableView);
var
  i: integer;
begin
...
Var
  TextToDraw: string;
begin
  TextToDraw := trim(AViewInfo.GridRecord.DisplayTexts[8]);
  if TextToDraw <> '' then
...
var
  filterText: string;
begin
  if (Length(trim(eFilter.Text)) > 0) //and (eFilter.Text <> filterText)
  then
  begin
    cxGrid1DBTableView1.DataController.DataModeController.SmartRefresh:=False;
    DataModule.DS_T_Users.Filtered := False;
...
begin
  ...
  begin
    cxGridDBTableView3.DataController.DataModeController.SmartRefresh:=False;
    DataModule.ds_product.Filtered := False;
  ...
  begin
    cxGridDBTableView3.DataController.DataModeController.SmartRefresh:=True;
    eFilterPrice.Clear;
    DataModule.ds_product.Filtered := False;
    DataModule.ds_product.ReopenLocate('TP_NAME');
  end;
...
procedure TMain.colCheckPricePropertiesEditValueChanged(Sender: TObject);
var
  ACheck: TcxCheckBox;
...
procedure TMain.colCheckPropertiesEditValueChanged(Sender: TObject);
var
  ACheck: TcxCheckBox;
  AGridSite: TcxGridSite;
  AGridView: TcxGridDBTableView;
...
var
  ...
  Cellvalue : integer;
begin
// ColumnID :=cxGrid1DBTableView1.GetColumnByFieldName('colCheck').Index;
...
if (AViewInfo.GridRecord.Values[colCheck.Index])=1 then
//with cxGrid1DBTableView1.Controller do
//    for i := 0 to SelectedRecordCount - 1 do
    ACanvas.Brush.Color := clHighlight;
//      Memo1.Lines.Add(VarToStr(cxGrid1DBTableView1.Controller.SelectedRecords[i].Values[0]));
//    if AGridView.DataController.Values[i, colCheck.Index] = true
end;

procedure TMain.cxGrid1DBTableView1FocusedRecordChanged(Sender:
    TcxCustomGridTableView; APrevFocusedRecord, AFocusedRecord:
    TcxCustomGridRecord; ANewItemRecordFocusingChanged: Boolean);
var
  AView: TcxGridDBTableView;
...
procedure TMain.cxGridDBTableView3MouseDown(Sender: TObject; Button:
...
procedure TMain.eFilterPriceClick(Sender: TObject);
begin
eFilterPrice.Text:='';
end;

procedure TMain.GroupPrintSpecProdExecute(Sender: TObject);
var
  Stream: TMemoryStream;
  User,S: string;
  TempPer, i: integer;
...
procedure TMain.UmCheck(var Message: TMessage);
begin
