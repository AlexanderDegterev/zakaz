object Remot_orders: TRemot_orders
  Left = 0
  Top = 0
  Caption = #1059#1076#1072#1083#1077#1085#1085#1099#1077' '#1079#1072#1082#1072#1079#1099
  ClientHeight = 635
  ClientWidth = 1030
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  PixelsPerInch = 96
  TextHeight = 13
  object Panel3: TPanel
    Left = 0
    Top = 0
    Width = 1030
    Height = 74
    Align = alTop
    TabOrder = 0
    object Label1: TLabel
      Left = 432
      Top = 11
      Width = 142
      Height = 13
      Caption = #1041#1040#1047#1059' '#1055#1045#1056#1045#1055#1054#1044#1050#1051#1070#1063#1048#1058#1068' !!!'
      Color = clRed
      ParentColor = False
      Visible = False
    end
    object ButtonClear: TBitBtn
      Left = 279
      Top = 38
      Width = 105
      Height = 25
      Caption = #1054#1095#1080#1089#1090#1080#1090#1100
      DoubleBuffered = True
      ParentDoubleBuffered = False
      TabOrder = 2
    end
    object eFilter: TEdit
      Left = 16
      Top = 40
      Width = 241
      Height = 21
      TabOrder = 0
    end
    object Panel1: TPanel
      Left = 16
      Top = 9
      Width = 241
      Height = 25
      Caption = #1042#1074#1077#1076#1080#1090#1077' '#1085#1086#1084#1077#1088' '#1079#1072#1082#1072#1079#1072' '#1076#1083#1103' '#1087#1086#1080#1089#1082#1072
      TabOrder = 1
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 594
    Width = 1030
    Height = 41
    Align = alBottom
    TabOrder = 1
    object ButtonEdit: TBitBtn
      Left = 16
      Top = 10
      Width = 105
      Height = 25
      Caption = #1056#1077#1076#1072#1082#1090#1080#1088#1086#1074#1072#1090#1100
      DoubleBuffered = True
      Glyph.Data = {
        
        00FFFB00FFFB00FFFB00FFFB00FFFB00FFFB00FFFB00FFFB00FF}
      ParentDoubleBuffered = False
      TabOrder = 0
      Visible = False
    end
    object ButtonDel: TBitBtn
      Left = 287
      Top = 10
      Width = 105
      Height = 25
      Caption = #1059#1076#1072#1083#1080#1090#1100
      DoubleBuffered = True
      Glyph.Data = {
        
        00FFFB00FFFB00FFFB00FFFB00FFFB00FFFB00FFFB00FFFB00FF}
      ParentDoubleBuffered = False
      TabOrder = 2
      Visible = False
    end
    object ButtonClose: TBitBtn
      Left = 899
      Top = 10
      Width = 105
      Height = 25
      Caption = #1047#1072#1082#1088#1099#1090#1100
      DoubleBuffered = True
      Glyph.Data = {
        
        00FFFB00FFFB00FFFB00FFFB00FFFB00FFFB00FFFB00FFFB00FF}
      ParentDoubleBuffered = False
      TabOrder = 1
      OnClick = ButtonCloseClick
    end
  end
  object cxGrid_Remote: TcxGrid
    Left = 0
    Top = 74
    Width = 1030
    Height = 520
    Touch.ParentTabletOptions = False
    Touch.TabletOptions = [toPressAndHold]
    Align = alClient
    TabOrder = 2
    ExplicitHeight = 481
    object cxGrid_RemoteDBTableView1: TcxGridDBTableView
      DataController.DataSource = DataModule.Source_USERS_DEL
      DataController.Summary.DefaultGroupSummaryItems = <>
      DataController.Summary.FooterSummaryItems = <>
      DataController.Summary.SummaryGroups = <>
      OptionsData.Editing = False
      OptionsData.Inserting = False
      OptionsSelection.HideSelection = True
      OptionsView.GroupByBox = False
      object cxGrid_RemoteDBTableView1U_ID: TcxGridDBColumn
        DataBinding.FieldName = 'U_ID'
        Visible = False
      end
      object cxGrid_RemoteDBTableView1U_NAME: TcxGridDBColumn
        DataBinding.FieldName = 'U_NAME'
        Visible = False
      end
      object cxGrid_RemoteDBTableView1CL_NAME: TcxGridDBColumn
        Caption = #1047#1072#1082#1072#1079#1095#1080#1082
        DataBinding.FieldName = 'CL_NAME'
        Width = 200
      end
      object cxGrid_RemoteDBTableView1U_NOMERZAK: TcxGridDBColumn
        Caption = #1053#1086#1084#1077#1088' '#1079#1072#1082#1072#1079#1072
        DataBinding.FieldName = 'U_NOMERZAK'
        Width = 84
      end
      object cxGrid_RemoteDBTableView1U_OBJECT: TcxGridDBColumn
        DataBinding.FieldName = 'U_OBJECT'
        Visible = False
        Width = 30
        IsCaptionAssigned = True
      end
      object cxGrid_RemoteDBTableView1OB_NAME: TcxGridDBColumn
        Caption = #1054#1073#1098#1077#1082#1090
        DataBinding.FieldName = 'OB_NAME'
        Width = 150
      end
      object cxGrid_RemoteDBTableView1U_DATEBEG: TcxGridDBColumn
        Caption = #1055#1086#1089#1090#1091#1087#1083#1077#1085#1080#1077
        DataBinding.FieldName = 'U_DATEBEG'
        Width = 62
      end
      object cxGrid_RemoteDBTableView1U_DATESTART: TcxGridDBColumn
        Caption = #1053#1072#1095#1072#1083#1086
        DataBinding.FieldName = 'U_DATESTART'
        Width = 62
      end
      object cxGrid_RemoteDBTableView1U_DATEEND: TcxGridDBColumn
        Caption = #1057#1088#1086#1082' '#1080#1089#1087#1086#1083#1085#1077#1085#1080#1103
        DataBinding.FieldName = 'U_DATEEND'
        Width = 62
      end
      object cxGrid_RemoteDBTableView1U_NASED: TcxGridDBColumn
        Caption = #1057#1090#1072#1090#1091#1089
        DataBinding.FieldName = 'U_NASED'
        Width = 134
      end
      object cxGrid_RemoteDBTableView1U_CERT: TcxGridDBColumn
        Caption = #1042#1077#1089' '#1080#1079#1076#1077#1083#1080#1081
        DataBinding.FieldName = 'U_CERT'
      end
      object cxGrid_RemoteDBTableView1U_MET: TcxGridDBColumn
        Caption = #1042#1077#1089' '#1084#1072#1090#1077#1088#1080#1072#1083#1086#1074
        DataBinding.FieldName = 'U_MET'
      end
      object cxGrid_RemoteDBTableView1U_PRIM: TcxGridDBColumn
        Caption = #1055#1088#1080#1084#1077#1095#1072#1085#1080#1077
        DataBinding.FieldName = 'U_PRIM'
        Width = 150
      end
      object cxGrid_RemoteDBTableView1U_PROFFIO: TcxGridDBColumn
        DataBinding.FieldName = 'U_PROFFIO'
        Visible = False
        Width = 150
      end
      object cxGrid_RemoteDBTableView1P_PROF: TcxGridDBColumn
        DataBinding.FieldName = 'P_PROF'
        Width = 150
      end
    end
    object cxGrid_RemoteLevel1: TcxGridLevel
      GridView = cxGrid_RemoteDBTableView1
    end
  end
end
